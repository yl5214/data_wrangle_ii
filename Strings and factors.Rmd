...
str_detect(string_vec, "i think$")
```
^i start with i think
think$ end with i think
...
str_detect(string_vec, "7\\.11")
```
.  represent any special charaters
\\. actual dot
```{r}
string_vec = c(
  'The CI is [2, 5]',
...
marj_df |> 
  filter(age == "18-25") |> 
  mutate(State = fct_reorder(State, percent)) |>  ## factor variable has order,if not mutate, alphabetical order for state (character), in order according to percentage because we want to see the pattern
